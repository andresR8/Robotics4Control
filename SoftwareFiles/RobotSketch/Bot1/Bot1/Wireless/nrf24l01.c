...
void nrf24l01_setirq() {
  ...
  config|= (1<<NRF24L01_REG_MASK_MAX_RT) | (1<< NRF24L01_REG_MASK_TX_DS); //turn Off IRQ on max_rt and Tx_ds
  config&= ~(1<<NRF24L01_REG_MASK_RX_DR); //turn on IRQ in RX
  nrf24l01_writeregister(NRF24L01_REG_CONFIG, config);
  
  //Configure uC interrupt to receive IRQ
   NRF24L01_IRQ_DDR &= ~(1<<NRF24L01_IRQ); // Extern interrupt INT0 Pin as input
   NRF24L01_IRQ_PORT |= (1<<NRF24L01_IRQ); // Extern interrupt INT0 Pin Pull Up
